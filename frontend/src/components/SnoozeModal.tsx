...
import { snoozeProperty } from '../api/client'
...
interface Props {
  ...
  onClose: () => void
  /** Called with the updated PropertyOut after a successful snooze. */
  onSnoozed: (prop: PropertyOut) => void
}
...
export default function SnoozeModal({ propertyId, propertyAddress, onClose, onSnoozed }: Props) {
  ...
  const handleConfirm = async () => {
    ...
    setError(null)
    try {
      const result = await snoozeProperty(propertyId, {
      ...
      setError(msg || 'Failed to snooze property')
      setSaving(false)
    }
  }
      ...
      <div className="bg-surface-card border border-surface-border rounded-2xl w-full max-w-md shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-surface-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-semibold text-ink-primary">Snooze Property</div>
              <div className="text-[11px] text-ink-muted truncate max-w-[260px]">{propertyAddress}</div>
            </div>
          </div>
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div>
            ...
            <input
              type="text"
              placeholder='e.g. "Under contract — PSA signed"'
              value={reason}
