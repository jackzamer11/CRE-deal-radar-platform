...
import { importCoStarTenants } from '../api/client'
import type { CoStarTenantImportResult } from '../api/client'
...
export default function CoStarTenantImportModal({ onClose, onDone }: Props) {
  const inputRef = useRef<HTMLInputElement>(null)
  const [file, setFile]             = useState<File | null>(null)
  const [uploading, setUploading]   = useState(false)
  const [result, setResult]         = useState<CoStarTenantImportResult | null>(null)
  const [apiError, setApiError]     = useState<string | null>(null)
  const [showErrors, setShowErrors] = useState(false)
  const [showUnmapped, setShowUnmapped] = useState(false)

  const handleFile = (f: File) => { setFile(f); setResult(null); setApiError(null) }
  ...
  const handleUpload = async () => {
    ...
    setResult(null)
    try {
      const res = await importCoStarTenants(file)
      setResult(res)
      if (res.inserted > 0 || res.updated > 0) onDone()
    } catch (e: any) {
      setApiError(
        e?.response?.data?.detail || e?.message || 'Import failed — check the server log for details.'
      )
    } finally {
      setUploading(false)
    }
      ...
      <div className="bg-surface-card border border-surface-border rounded-2xl shadow-2xl w-full max-w-xl mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-surface-border sticky top-0 bg-surface-card z-10">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-sm font-bold text-ink-primary">Import CoStar Tenant Locations</h2>
          </div>
        ...
        <div className="p-6 space-y-5">
          ...
          {!result && (
            <div className="text-xs text-ink-muted bg-surface-muted rounded-lg p-3 space-y-1">
              <p className="font-semibold text-ink-secondary">Required CoStar columns (18):</p>
              <p>Address · Tenant Name · Industry · Employees · Website · Submarket ·
                 SF Occupied · NAICS · City · State · Zip · Best Tenant Contact ·
                 Best Tenant Phone · Tenant Representative · Next Break Date ·
                 Rent/SF/year · Future Move · Future Move Type</p>
              <p className="text-[10px] mt-1 text-ink-muted">
                Rows filtered to VA + mapped NoVA submarkets + SF Occupied ≥ 2,500 only.
              </p>
            </div>
          )}
            ...
            <div
              ...
              className={`
                ...
                ${file
                  ...
                  : 'border-surface-border hover:border-emerald-400/40 hover:bg-surface-muted'}
              `}
            >
              <input
                ref={inputRef}
              ...
              />
              <Upload size={22} className={`mx-auto mb-3 ${file ? 'text-emerald-400' : 'text-ink-muted'}`} />
              {file ? (
                <>
                ...
                <>
                  <p className="text-sm text-ink-secondary">Drop a CoStar Tenant Locations .csv or .xlsx export here</p>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-surface-muted rounded-lg p-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <FilterRow label="Removed — SF Occupied < 2,500" count={result.filtered_size}      color="text-amber-400" />
                </div>
              </div>
              ...
              {(result.inserted > 0 || result.updated > 0) && (
                <div className="flex items-center gap-2 text-xs text-emerald-400">
                  <CheckCircle size={13} />
                  Signals computed. Update headcount history and open positions for full scoring.
                </div>
              )}
                  ...
                  {showUnmapped && (
                    <div className="mt-2 rounded-lg border border-surface-border bg-surface-muted p-3">
                      <p className="text-[10px] text-ink-muted mb-2">
                        Add these to COSTAR_SUBMARKET_MAP in companies.py to import them next time:
                      </p>
        ...
        {!result && (
          <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-surface-border">
            <button onClick={onClose} className="px-4 py-2 text-xs text-ink-muted hover:text-ink-primary transition-colors">
              Cancel
            </button>
            <button
...
function StatBox({ value, label, color }: { value: number; label: string; color: string }) {
  ...
  return (
    <div className={`rounded-lg border p-3 text-center ${colorMap[color] ?? colorMap.muted}`}>
